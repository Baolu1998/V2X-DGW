...
def nms_rotated(boxes, scores, threshold):
    ...
    while len(ixs) > 0:
        ...
        # remove_ixs_2 = np.where(np.logical_and(fin_iou < 0.3, fin_iou != 0))[0] + 1
        # remove_ixs_total = np.concatenate((remove_ixs,remove_ixs_2))
        # remove_ixs = np.unique(remove_ixs_total)
...
def v2v_nms_rotated(boxes, scores, threshold):
    """Performs rorated non-maximum suppression and returns indices of kept
    boxes.
    ...
    while len(ixs) > 0:
        ...
        remove_ixs = np.where(iou > threshold)[0] + 1
        if len(remove_ixs) == 0:
            single_pick.append(i)
        # Remove indices of the picked and overlapped boxes.
        ixs = np.delete(ixs, remove_ixs)
        ixs = np.delete(ixs, 0)

    return np.array(pick, dtype=np.int32),np.array(single_pick, dtype=np.int32)
