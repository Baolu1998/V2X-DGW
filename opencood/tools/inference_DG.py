...
# def test_parser():
#     parser = argparse.ArgumentParser(description="synthetic data generation")
#     parser.add_argument('--model_dir', type=str,default='/home/baoluli/personal/2.model_saved/4.Adverseweather/v2xset_DG/point_pillar_intermediate_fusion_all_2024_02_24_13_19_54', #required=True,# 
#                         help='Continued training path')
...
def main():
    ...
    for item in hypes['DG_params']['DG_validate_dir']:
        DG_validate_datasets.append(build_dataset(hypes, visualize=False, train=False, isSim=opt.isSim,DG_val_dir=item))
    DG_val_loaders = []
    for item in DG_validate_datasets:
        DG_val_loaders.append(DataLoader(item,
                            batch_size=1,
                            num_workers=0,
                            collate_fn=item.collate_batch_test,
                            shuffle=False,
                            pin_memory=False,
                            drop_last=False))
    ...
    for idx in range(hypes['DG_params']['DG_validate_num']):
        ...
        #         result_stat_DGs.append({0.5: {'tp': [], 'fp': [], 'gt': 0},
        #             0.7: {'tp': [], 'fp': [], 'gt': 0}})



        cur_loader = DG_val_loaders[idx]
        result_stat = {0.5: {'tp': [], 'fp': [], 'gt': 0},
                0.7: {'tp': [], 'fp': [], 'gt': 0}}
        cur_valid_set = hypes['DG_params']['DG_validate_name'][idx]
