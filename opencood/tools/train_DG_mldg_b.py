...
# def train_parser():
#     parser = argparse.ArgumentParser(description="synthetic data generation")
#     parser.add_argument("--hypes_yaml", type=str, default='/home/baoluli/1.code/4.Adverseweather/V2V_baolu/opencood/hypes_yaml/opv2v_DG/point_pillar_intermediate_fusion_mldg(b).yaml',#required=True,
#                         help='data generation yaml file needed ')
#     parser.add_argument('--model_dir', type=str, default='',#required=True,#,
#                         help='Continued training path')
#     parser.add_argument("--half", action='store_true',
...
def main():
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            if cur_data_num < 3:
                cur_data_num += 1
                cur_data_list.append(batch_data)
                continue
            else:
                cur_data_list.append(batch_data)
                ...
                model.zero_grad()
                optimizer.zero_grad()
                batch_data_1 = cur_data_list[0]
                batch_data_2 = cur_data_list[1]
                batch_data_3 = cur_data_list[2]
                batch_data_4 = cur_data_list[3]

                batch_data_1 = train_utils.to_device(batch_data_1, device)
                batch_data_2 = train_utils.to_device(batch_data_2, device)
                batch_data_3 = train_utils.to_device(batch_data_3, device)
                batch_data_4 = train_utils.to_device(batch_data_4, device)
                ...
                # as well
                if not opt.half:
                    ouput_dict_1 = model(batch_data_1['ego'])
                    ouput_dict_2 = model(batch_data_2['ego'])
                    ouput_dict_3 = model(batch_data_3['ego'])
                    ouput_dict_4 = model(batch_data_4['ego'])
                    # first argument is always your output dictionary,
                    # second argument is always your label dictionary.
                    final_loss_1 = criterion(ouput_dict_1,
                                        batch_data_1['ego']['label_dict'])
                    final_loss_2 = criterion(ouput_dict_2,
                                        batch_data_2['ego']['label_dict'])
                    final_loss_3 = criterion(ouput_dict_3,
                                        batch_data_1['ego']['label_dict'])
                    final_loss_4 = criterion(ouput_dict_4,
                                        batch_data_2['ego']['label_dict'])
                else:
                ...
                selected_vector = ouput_dict_1['spatial_features_2d'] #torch.randn(4)
                min_distance = float('inf')
                closest_vector_index = -1

                vectors = [ouput_dict_2['spatial_features_2d'],ouput_dict_3['spatial_features_2d'],ouput_dict_4['spatial_features_2d']]
                import torch.nn.functional as F
                for i, vector in enumerate(vectors):
                    # 计算余弦相似度
                    
                    similarity = F.cosine_similarity(selected_vector.view(1,-1), vector.view(1,-1))
                    # 更新最小距离和对应的向量索引
                    if similarity < min_distance:
                        min_distance = similarity
                        closest_vector_index = i
                if closest_vector_index == 0:
                    final_loss = (final_loss_1 + final_loss_2) + 0.1 * (final_loss_3 + final_loss_4)
                elif closest_vector_index == 1:
                    final_loss = (final_loss_1 + final_loss_3) + 0.1 * (final_loss_2 + final_loss_4)
                elif closest_vector_index == 2:
                    final_loss = (final_loss_1 + final_loss_4) + 0.1 * (final_loss_2 + final_loss_3)
