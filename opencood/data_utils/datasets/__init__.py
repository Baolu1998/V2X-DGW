...
def build_dataset(dataset_cfg, visualize=False, train=True, isSim=False,DG_val_dir=None):

    data_full_path = dataset_cfg['root_dir']
    if 'opv2v' in data_full_path:
        isSim = True
        print("opv2v dataset loading")
    elif 'V2X' in data_full_path:
        isSim = True
        print("V2XSet dataset loading")
    else:
        print("v2v4real dataset loading")
